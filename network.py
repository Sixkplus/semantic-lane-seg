...
class Network(object):
    ...
    def save(self, saver, save_dir, step):
        ...
        checkpoint_path = os.path.join(save_dir, dataset, model_name)
    ...
    @layer
    def reduce_mean(self, input, axis, keepdims, name):
        return tf.reduce_mean(input, axis, keep_dims=keepdims, name=name)
    
    @layer
    def multiply(self, input, y, name):
        return tf.multiply(input, y, name=name)
    ...
    @layer
    def batch_normalization(self, input, name, scale_offset=True, relu=False, bn_momentum=0.9):
