...
def create_losses(net, label, cfg, isLane = False):
            ...
            with tf.device('/gpu:%d' % d):
                with tf.name_scope('tower_%d' % d):
                    ...
                    with tf.name_scope('Seg'):
                        ...
                        loss_cls, reduced_loss1 = create_losses(train_net, label_splits[counter], cfg, isLane=False)
                    tf.get_variable_scope().reuse_variables()
                    with tf.name_scope('Lane'):
                        train_net = model(next_image = lane_image_splits[counter], cfg=cfg, mode='train')
                        loss_lane, reduced_loss2 = create_losses(train_net, lane_label_splits[counter], cfg, isLane=True)
