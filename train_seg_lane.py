...
def create_losses(net, label, label_lane, cfg):
    if cfg.AUX_LOSSES == False:
        ...
        loss_lane = create_loss(pred_lane, label_lane, 2, cfg.param['ignore_label'], cfg.USE_WEIGHT)
    ...
    reduced_loss = loss_cls + tf.add_n(l2_losses) + 0.5*loss_lane
    return loss_cls, loss_lane, reduced_loss
...
def main():
    ...
    next_img, next_label, next_lane = train_reader.next_image, train_reader.next_label, train_reader.next_lane
    image_splits = tf.split(next_img, cfg.num_gpu)
    label_splits = tf.split(next_label, cfg.num_gpu)
    lane_splits = tf.split(next_lane, cfg.num_gpu)
    tower_grads = []
    tower_loss = []
    tower_loss_cls = []
    tower_loss_lane = []
        ...
        for d in range(cfg.num_gpu):
            with tf.device('/gpu:%d' % d):
                with tf.name_scope('tower_%d' % d):
                    ...
                    loss_cls, loss_lane, reduced_loss = create_losses(train_net, train_net.labels, lane_splits[counter], cfg)
                    counter += 1
                    # Create loss
